<div align="center">

# ✨ Adithya D — Data Analyst Portfolio

![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&pause=1000&color=F7B731&center=true&vCenter=true&width=500&lines=Data+Analyst+%7C+Python+%7C+SQL;Power+BI+%7C+Tableau+%7C+ML;Turning+Data+into+Insights+🚀)

[![Portfolio](https://img.shields.io/badge/🌐_Live_Portfolio-Visit_Now-brightgreen?style=for-the-badge)](https://adithyadevadas.netlify.app)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/adithya-devadas)
[![GitHub](https://img.shields.io/badge/GitHub-Follow-181717?style=for-the-badge&logo=github)](https://github.com/adithyadas01)
[![Email](https://img.shields.io/badge/Email-Contact-D14836?style=for-the-badge&logo=gmail)](mailto:adithyadevadas01@gmail.com)

</div>

---

## 🙋‍♂️ About Me

> *"Results-driven Data Analyst passionate about transforming raw data into strategic business insights."*

🎓 B.Sc. Computer Science — NSS College Ottapalam, University of Calicut (2025)  
📍 Palakkad, Kerala, India  
📊 Skilled in the full data lifecycle — from cleaning & EDA to ML and dashboards

---

## 🛠️ Tech Stack

| Category | Tools |
|----------|-------|
| 💻 Languages | Python, SQL, R |
| 📊 Visualization | Power BI, Tableau, Excel |
| 🤖 ML & Data | Scikit-learn, Pandas, NumPy |
| 🌐 Web & Deploy | Streamlit, HTML, Netlify |

---

## 💼 Experience

🏢 **Data Analyst Intern** — Techolas Technologies, Palakkad
`Jul 2025 – Present`
- Performed data cleaning on 10K+ records using Python & SQL
- Built interactive Power BI dashboards for KPI tracking
- Optimised SQL queries improving data retrieval efficiency by 20%

🏢 **Python Programming Intern** — CodeAlpha, Remote
`Oct 2025 – Nov 2025`
- Automated data processing workflows using Python scripts
- Improved script efficiency through systematic debugging

🏢 **Data Analytics Virtual Internship** — Deloitte (Forage)
- Completed job simulation in data analytics and forensic technology

---

## 🚀 Projects

| Project | Description | Tools |
|---------|-------------|-------|
| 📊 Data Analysis Projects | EDA, trend analysis, reporting | Python, Pandas |
| 📈 BI Dashboards | Interactive KPI dashboards | Power BI, Tableau |
| 🤖 ML Models | Predictive analytics | Scikit-learn, Python |

---

## 📜 Certifications

- 🏅 Deloitte Data Analytics Virtual Internship — Forage
- 🏅 Python & SQL Certifications

---

## 📫 Let's Connect!

<div align="center">

| Platform | Link |
|----------|------|
| 🌐 Portfolio | [adithyadevadas.netlify.app](https://adithyadevadas.netlify.app) |
| 💼 LinkedIn | [linkedin.com/in/adithya-devadas](https://www.linkedin.com/in/adithya-devadas) |
| 🐙 GitHub | [github.com/adithyadas01](https://github.com/adithyadas01) |
| 📧 Email | adithyadevadas01@gmail.com |

</div>

---

<div align="center">

⭐ **If you like my portfolio, give this repo a star!** ⭐

[![Portfolio Live](https://img.shields.io/badge/🚀_View_My_Portfolio-adithyadevadas.netlify.app-FF6B6B?style=for-the-badge)](https://adithyadevadas.netlify.app)

</div>
